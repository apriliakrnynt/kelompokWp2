...
class Order extends MY_Controller
{
    ...
    public function saveitem()
    {
        ...
        $checkMenu = $this->db->get_where('food_order_detail', array('menu_id' => $menu_id, 'order_id' => $order_id))->row();
    ...
    {
        $data = $this->db->select_sum('order_detail_waktu', 'waktu')->select_sum('order_detail_qty', 'qty')->select_sum('order_detail_subtotal', 'total')->get_where('food_order_detail', array('order_id' => $order_id))->row();
    ...
    public function bayar()
    {
        ...
        $dataJumlah = $this->db->select_sum('order_detail_qty', 'qty')->get_where('food_order_detail', array('order_id' => $order_id))->row();
        $qty        = $dataJumlah->qty;
        $data       = array(
        ...
        $this->db->update('food_order', $data);
        ...
        $this->db->where('order_id', $order_id);
        $this->db->update('food_order_detail', $dataDetail);

        // Item Order
        $listItem = $this->db->get_where('food_order_detail', array('order_id' => $order_id))->result();
        foreach ($listItem as $i) {
            $menu_id  = $i->menu_id;
            $Jual     = $this->db->get_where('food_menu', array('menu_id' => $menu_id))->row();
            $dataMenu = array(
                'menu_jual' => ($Jual->menu_jual + $i->order_detail_qty),
            );

            $this->db->where('menu_id', $menu_id);
            $this->db->update('food_menu', $dataMenu);
        }
    ...
    {
        // $dataToko    = $this->db->get_where('food_contact', array('contact_id' => 1))->row();
        ...
        // $printer        = $this->db->get_where('food_printer', array('printer_tipe' => 'Nota'))->row();
